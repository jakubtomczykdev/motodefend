...
func _ready() -> void:
	...
	# Jawne warstwy kolizji dla niezawodnej detekcji przez Area2D (pociski)
	collision_layer = 1
	collision_mask = 1
...
func take_damage(amount: int) -> void:
	...
	print("[Enemy] %s took %d dmg | HP: %d/%d" % [name, amount, current_health, max_health])
	damaged.emit(amount)

	# Wizualny feedback – krótki czerwony flash
	_flash_damage()
