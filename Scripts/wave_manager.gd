...
func _process(delta: float) -> void:
	if is_wave_active:
		...
		# Fala trwa aż do eliminacji wszystkich wrogów – brak sztucznego limitu czasu
...
func start_next_wave() -> void:
	...
	var living_enemies: int = get_tree().get_nodes_in_group("Enemies").size()
	enemies_in_wave = wave_config.enemy_count + living_enemies
...
func update_timer_ui() -> void:
	if timer_ui:
		timer_ui.text = "CZAS FALI: %.1fs" % wave_elapsed
