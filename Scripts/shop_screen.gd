...
func _create_item_button(item: ItemBase) -> void:
	...
	btn.pressed.connect(_on_item_bought.bind(item, btn))
	items_container.add_child(btn)

func _on_item_bought(item: ItemBase, btn: Button) -> void:
	...
	btn.disabled = true
	btn.text = "%s – KUPIONE" % item.item_name

func _update_gold_label() -> void:
	gold_label.text = "GOLD: %d" % player_gold

func open_shop(current_gold: int) -> void:
